...
class VEMModel(PickleFileIO):
    ...
    def run_one_iteration(self):
        # theta - vector of topic proportions for a particular document
        # phi - topics -- size vocab-1; we have k topics

        # E - expectation step - how likely is each point to fit this distribution
        # variational alpha - parameter vector for Dirichlet document-level topics distribution
        # of topics x # of documents
        log.debug('Updating vAlpha')
        self.update_valpha()

        # variational mu - global topic distribution parameter - variational topic means
        log.debug('Updating vMu')
        self.update_vmu()

        # variational M - global topic distribution parameter - variational topic means
        log.debug('Updating vM')
        self.update_vm()

        # M - maximization step - fix the distributions to better match expectations
        log.debug('Updating M')
        self.update_m()

        # xi - global topic distribution parameter - variational topic means
        log.debug('Updating xi')
        self.update_xi()

        # vector dimension = topics
    ...
    def write_topic_weights_arff(self, f=None):
        ...
        mean_topic_weights = self.valpha / asrxowvector(np.sum(self.valpha, axis=0))
