...
class CorpusReader:
    ...
    def init_labels(self):
        ...
        self.labels = np.array([self.class_names.index(each) for each in self.raw_labels],
                               dtype='int8')  # Class indices
...
def corpus_mean_and_precision(reader):
    ...
    return mean, reader.dim * reader.num_data / (
                sum_squared_norms - 2 * mean.dot(sum) + reader.num_data * squared_norm_mean)
